...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const employee of employees) {
      ...
      // Calculate weekly overtime (separate from daily pattern mismatches)
      const weeklyOvertimeThreshold = currentPattern?.totalHoursPerWeek || 40;
      const weeklyOvertime = Math.max(0, actualHours - weeklyOvertimeThreshold);

      // For permanent employees, overtime can become compensation hours (1:1 ratio)
      // Include both pattern discrepancy overtime and weekly overtime
      const totalCompensatableOvertime = Math.max(overtime, weeklyOvertime);
      const compensationHours = employee.employeeType === 'PERMANENT' ? totalCompensatableOvertime : 0;
